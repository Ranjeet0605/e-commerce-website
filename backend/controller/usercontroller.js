...
exports.loginUser = catchAsyncError(async(req,res,next)=>{
    ...
    // cheking if user give passowrd and email
   if(!email || !password){
    return next(new ErrorHandler("Please Enter the email and password", 400))
   }
  const user = await userSchema.findOne({email}).select("+password")
  if(!user){
    return next(new ErrorHandler("your password and email is invalid", 400))
  }
  const isPasswordMatched = await user.comparePassword(password);
  if(!isPasswordMatched){
    return next(new ErrorHandler("your passoword and email is invalid",401))
...
// get all users (admin can see this details all users)
